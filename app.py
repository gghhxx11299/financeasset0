...
class RobustHDFM:
    ...
    def _calculate_atr(self, high, low, close, length=14):
        """Robust ATR calculation with proper input handling"""
        try:
            ...
            hl = high - low
            hc = np.abs(high[1:] - close[:-1])  # high - previous close
            lc = np.abs(low[1:] - close[:-1])   # low - previous close
            
            # Pad the shifted arrays to match original length
    ...
    def forecast(self, current_price, metrics):
        """Generate price targets with regime adjustment"""
        try:
    ...
    def adaptive_error_correction(self, current_price, predictions, actual_future):
        """Adjust predictions based on historical errors"""
        try:
...
def main():
    ...
    st.title("🔒 Robust HDFM Trading System (Fixed Version)")
                ...
                try:
                    ...
                    if model.circuit_breaker:
                        ...
                        if not up_signals.empty:
                            ax.scatter(up_signals.index, up_signals['Close'], 
                        ...
                        if not down_signals.empty:
                            ax.scatter(down_signals.index, down_signals['Close'], 
